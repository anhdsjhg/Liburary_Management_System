...
export function useAnnouncementDetail() {
  ...
  const title = computed(() => {
    ...
    return (
      announcement.value[`title_${lang}`] ?? announcement.value.title_en ?? ""
    );
  });
  ...
  const place = computed(() => {
    ...
    const lang = locale.value as "en" | "ru" | "kz";
    return (
      announcement.value[`place_${lang}`] ?? announcement.value.place_en ?? ""
    );
  });
  ...
  const description = computed(() => {
    ...
    const lang = locale.value as "en" | "ru" | "kz";
    return (
      announcement.value[`description_${lang}`] ??
      announcement.value.description_en ??
      ""
    );
  });
