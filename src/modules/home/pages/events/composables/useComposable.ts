...
const EMPTY_RESULT: EventSearchResult = {
  data: [],
  total: 0,
  per_page: 10,
  current_page: 1,
...
export function useAllEventsSearch() {
  const { locale } = useI18n();
  const { mutate, isPending: isLoading, data: mutationData } = useAnnouncementSearchApi();
  ...
  const results = computed<EventSearchResult>(
    () => mutationData.value?.res ?? EMPTY_RESULT
  );
